...
double temperature, setPoint=32, controlSig;
...
bool updateTemperature() {
  if ((millis() - lastTempUpdate) > TEMP_READ_DELAY) {
    ...
    lastTempUpdate = millis();
      ...
      if(temperature>=setPoint && fanStatus==0)
      {
          digitalWrite(FANPIN,HIGH);
          Serial.println("Fan On");
          fanStatus=1;
      }else if(temperature<setPoint-margin && fanStatus==1){
...
//  digitalWrite(FANPIN,HIGH); // Pin D0 is HIGH
//  delay(1000);
//  digitalWrite(FANPIN,LOW); // Pin D0 is LOW
//  delay(1000);
